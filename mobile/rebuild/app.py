...
from .workspace import RebuildDashboard, RebuildWorkspace, RebuildTable, CrudForm
from .actions import SchoolActionScreen

class FrahooshRebuildApp(App):
    def __init__(self,**kwargs):
        super().__init__(**kwargs); self.app_state=None; self.sm=None; self._workspace=None; self._overlay=None

    def build(self):
        self.title="فراهوش"
        self.sm=ScreenManager(transition=FadeTransition(duration=.14))
    ...
    def _go(self,name,direction="left"):
        self.sm.transition=SlideTransition(duration=.18,direction=direction)
    ...
    def open_dashboard(self):
        try:
            ...
            self._go("dashboard","left"); return True
        except Exception as exc:
            print("REBUILD DASHBOARD ERROR:",repr(exc)); return False

    def show_dashboard(self):
        self._go("dashboard","right")
    ...
    def open_workspace(self,panel_key="management"):
        ...
        screen=self.sm.get_screen("workspace"); screen.panel_key=panel_key; screen._build()
        self._go("workspace","left")

    def show_workspace(self):
        self._go("workspace","right")
    ...
    def open_overlay(self,screen):
        self.close_overlay()
        self._overlay=screen
        self.sm.add_widget(screen)
        self._go(screen.name,"left")

    def close_overlay(self):
        if self._overlay is not None and self.sm.has_screen(self._overlay.name):
            name=self._overlay.name
            self.sm.remove_widget(self._overlay)
            self._overlay=None
        self._go("workspace","right")
