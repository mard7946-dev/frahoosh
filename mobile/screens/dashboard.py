...
class DashboardScreen(Screen):
    """Stable dashboard that never depends on optional graphics/assets."""
    ...
    def _build_ui(self):
        ...
        self.logout_button = Button(
            ...
            font_size="15sp",
            background_normal="",
            background_color=(0.65, 0.12, 0.14, 1),
            color=WHITE,
            size_hint_y=None,
            height=dp(48),
        )
        self.logout_button.bind(on_release=self.logout)
        root.add_widget(self.logout_button)
    ...
    def _get_display_name(self):
        try:
            value = self.app_state.display_name
            return str(value or "کاربر فراهوش")
        except Exception as exc:
    ...
    def refresh(self):
        ...
        title = ROLE_TITLES.get(role, "کاربر")

        self.welcome_label.text = rtl_text(f"خوش آمدید، {name}")
        self.role_label.text = rtl_text(f"نقش کاربری: {title}")
    ...
    def _populate_menu(self, role):
        ...
        items = MANAGER_MENU if role == "manager" else ROLE_MENU.get(
            role,
            [("صندوق پیام‌ها", "messages"), ("درباره برنامه", "about")],
        )
        ...
        for title, route in items:
            button = Button(
                ...
                font_size="15sp",
                background_normal="",
                background_color=(0.08, 0.30, 0.48, 1),
                color=WHITE,
                size_hint_y=None,
                height=dp(50),
            )
        ...
        try:
            if not self.manager.has_screen("module"):
                ...
                self.manager.add_widget(
                    ModuleScreen(name="module", app_state=self.app_state)
                )
