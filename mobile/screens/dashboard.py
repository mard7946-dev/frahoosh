...
ROLE_ALIASES = {
    ...
    "cultural": "cultural", "پرورشی": "cultural", "معاون پرورشی": "cultural", "advisor": "advisor", "counselor": "advisor", "مشاور": "advisor", "مشاوره": "advisor",
...
MANAGER_MENU = [
    ...
    ("گزارش‌ها", "reports"), ("صندوق پیام‌ها", "messages"), ("تنظیمات", "settings"), ("درباره برنامه", "about"),
]
ROLE_MENU = {
    ...
    "educational": [("معاون آموزشی", "educational"), ("دانش‌آموزان", "students"), ("دبیران", "teachers"), ("کلاس‌های آنلاین", "online"), ("تابلو هوشمند", "smart_board"), ("گزارش‌ها", "reports"), ("صندوق پیام‌ها", "messages"), ("تنظیمات", "settings"), ("درباره برنامه", "about")],
    "cultural": [("معاون پرورشی", "cultural"), ("دانش‌آموزان", "students"), ("اولیا", "parents"), ("پرداخت آنلاین", "payment"), ("تابلو هوشمند", "smart_board"), ("گزارش‌ها", "reports"), ("صندوق پیام‌ها", "messages"), ("تنظیمات", "settings"), ("درباره برنامه", "about")],
    ...
    "teacher": [("پنل دبیر", "teacher"), ("دانش‌آموزان", "students"), ("کلاس‌های آنلاین", "online"), ("تابلو هوشمند", "smart_board"), ("صندوق پیام‌ها", "messages"), ("تنظیمات", "settings"), ("درباره برنامه", "about")],
...
class DashboardScreen(Screen):
    ...
    def _build_ui(self):
        ...
        root.add_widget(self.welcome_label); root.add_widget(self.role_label); root.add_widget(self.status_label)
        scroll = ScrollView(do_scroll_x=False)
        self.menu_box = GridLayout(cols=1, spacing=dp(9), padding=[dp(2), dp(8), dp(2), dp(8)], size_hint_y=None)
        self.menu_box.bind(minimum_height=self.menu_box.setter("height")); scroll.add_widget(self.menu_box); root.add_widget(scroll)
        ...
        logout_button.bind(on_release=self.logout); root.add_widget(logout_button); self.add_widget(root)

    def _get_role(self):
        try: raw = self.app_state.role
        except Exception as exc: print("DASHBOARD ROLE ERROR:", repr(exc)); raw = "student"
    ...
    def _get_display_name(self):
        ...
        except Exception as exc: print("DASHBOARD NAME ERROR:", repr(exc)); return "کاربر فراهوش"

    def refresh(self):
        role = self._get_role(); name = self._get_display_name(); title = ROLE_TITLES.get(role, "کاربر")
        self.welcome_label.text = rtl_text(f"خوش آمدید، {name}")
        self.role_label.text = rtl_text(f"ورود موفق به پنل {title} | {SCHOOL_NAME}")
        self.status_label.text = rtl_text(f"{name} عزیز، امکانات اختصاصی پنل {title} در اختیار شماست.")
        self._populate_menu(role); return True
    ...
    def _populate_menu(self, role):
        ...
        for title, route in items:
            ...
            button.bind(on_release=lambda btn, r=route: self._menu_selected(r)); self.menu_box.add_widget(button)
    ...
    def _open_operations(self, route):
        ...
        screen = self.manager.get_screen("operations"); screen.set_route(route); self.manager.current = "operations"
    ...
    def _menu_selected(self, route):
        ...
        try:
            ...
            if route in LIVE_ROUTES:
                ...
                panel = self.manager.get_screen("live_panel"); panel.set_panel(route); self.manager.current = "live_panel"; return
            ...
            if hasattr(module, "set_module"): module.set_module(route)
            elif hasattr(module, "load_module"): module.load_module(route)
