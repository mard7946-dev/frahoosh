...
class SwipeArea(FloatLayout):
    ...
    def set_pages(self,pages):
        ...
        self._notify()
    def _mount(self,page): page.size_hint=(1,1); page.pos_hint={"x":0,"y":0}; self.add_widget(page)
    def _notify(self):
    ...
    def on_touch_up(self,touch):
        ...
        if self._touch_start is not None:
            ...
            if abs(dy)>dp(35) and abs(dy)>abs(dx)*1.15:
                self.next() if dy<0 else self.previous(); return True
        return handled

class DashboardScreen(Screen):
    def __init__(self,app_state=None,**kwargs): super().__init__(**kwargs); self.app_state=app_state; self.drawer_open=False; self._build_ui()
    ...
    def _build_ui(self):
        ...
        content=BoxLayout(orientation="vertical",padding=dp(10),spacing=dp(6)); header=BoxLayout(size_hint_y=None,height=dp(56),spacing=dp(8))
        ...
        self.status=self._label("","9sp",SUCCESS,True,True); content.add_widget(self.status); content.add_widget(self._label("پنل‌های شما","14sp",PRIMARY,True,True)); self.frame=PanelCard(size_hint_y=1,padding=dp(8)); self.deck=SwipeArea(size_hint=(1,1)); self.deck.on_change=self._deck_changed; self.frame.add_widget(self.deck); content.add_widget(self.frame); self.counter=self._label("","9sp",SUCCESS,True,True); content.add_widget(self.counter); content.add_widget(self._label(f"{SCHOOL_NAME} | سال تحصیلی {SCHOOL_YEAR or '۱۴۰۵-۱۴۰۶'}","8sp",SECONDARY,False,True))
        ...
        self.drawer_layer=FloatLayout(size_hint=(1,1),opacity=0,disabled=True); self.drawer_layer.add_widget(Button(background_normal="",background_color=(0,0,0,.28),size_hint=(1,1),on_release=self.close_drawer)); self.drawer=BoxLayout(orientation="vertical",padding=dp(10),spacing=dp(5),size_hint=(None,1),width=dp(280),pos_hint={"right":1}); self.drawer.add_widget(self._label(APP_NAME,"18sp",PRIMARY,True,True)); from kivy.uix.scrollview import ScrollView; scroll=ScrollView(do_scroll_x=False); self.drawer_box=BoxLayout(orientation="vertical",spacing=dp(4),size_hint_y=None); self.drawer_box.bind(minimum_height=self.drawer_box.setter("height")); scroll.add_widget(self.drawer_box); self.drawer.add_widget(scroll); close=Button(text=rtl_text("بستن منو"),font_name=font_name(),background_normal="",background_color=PRIMARY,color=WHITE,size_hint_y=None,height=dp(38)); close.bind(on_release=self.close_drawer); self.drawer.add_widget(close); self.drawer_layer.add_widget(self.drawer); root.add_widget(self.drawer_layer); self.add_widget(root)
    def role(self):
