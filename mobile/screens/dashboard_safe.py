...
class SwipeArea(FloatLayout):
    ...
    def set_pages(self, pages):
        self.clear_widgets(); self.pages = list(pages or []); self.index = 0
        if self.pages: self._mount(self.pages[0])
        self._notify()
    def _mount(self, page):
        page.size_hint = (1, 1); page.pos_hint = {"x": 0, "y": 0}; self.add_widget(page)
    def _notify(self):
        if callable(self.on_change): self.on_change(self.index, len(self.pages))
    def show(self, index):
        if not self.pages: return
        self.index = max(0, min(int(index), len(self.pages)-1)); self.clear_widgets(); self._mount(self.pages[self.index]); self._notify()
    def next(self): self.show(self.index + 1)
    def previous(self): self.show(self.index - 1)
    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos): self._touch_start = touch.pos
        return super().on_touch_down(touch)
    def on_touch_up(self, touch):
        handled = super().on_touch_up(touch)
        if self._touch_start is not None:
            dx = touch.x - self._touch_start[0]; dy = touch.y - self._touch_start[1]; self._touch_start = None
            if abs(dy) > dp(35) and abs(dy) > abs(dx) * 1.15:
                self.next() if dy < 0 else self.previous()
                return True
        return handled
...
class DashboardScreen(Screen):
    ...
    def _label(self, text, size="13sp", color=SECONDARY, bold=False, center=False):
        w = Label(text=rtl_text(text), font_name=font_name(), font_size=size, color=color, bold=bold, halign="center" if center else "right", valign="middle")
        w.bind(size=lambda o, v: setattr(o, "text_size", v)); return w
    def _build_ui(self):
        root = FloatLayout()
        with root.canvas.before: Color(.94, .97, .985, 1); self.bg = RoundedRectangle()
        ...
        menu = Button(text="☰", font_size="25sp", background_normal="", background_color=PRIMARY, color=WHITE, size_hint_x=None, width=dp(50)); menu.bind(on_release=self.toggle_drawer); header.add_widget(menu)
        try: header.add_widget(Image(source=LOGO_PATH, size_hint_x=None, width=dp(48), allow_stretch=True, keep_ratio=True))
        except Exception: pass
        titles = BoxLayout(orientation="vertical"); titles.add_widget(self._label(APP_NAME,"19sp",PRIMARY,True)); titles.add_widget(self._label("سامانه هوشمند آموزشی یکپارچه مدرسه","9sp")); header.add_widget(titles); content.add_widget(header)
        welcome = PanelCard(size_hint_y=None, height=dp(76), padding=dp(10)); self.welcome=self._label("خوش آمدید","18sp",PRIMARY,True); self.role_text=self._label("","10sp",SECONDARY,True); self.school_text=self._label(SCHOOL_NAME,"10sp",PRIMARY,True); welcome.add_widget(self.welcome); welcome.add_widget(self.role_text); welcome.add_widget(self.school_text); content.add_widget(welcome)
        self.status = self._label("","9sp",SUCCESS,True,True); content.add_widget(self.status); content.add_widget(self._label("پنل‌های شما","14sp",PRIMARY,True,True))
        self.frame = PanelCard(size_hint_y=1, padding=dp(8)); self.deck=SwipeArea(size_hint=(1,1)); self.deck.on_change=self._deck_changed; self.frame.add_widget(self.deck); content.add_widget(self.frame)
        self.counter=self._label("","9sp",SUCCESS,True,True); content.add_widget(self.counter); content.add_widget(self._label(f"{SCHOOL_NAME} | سال تحصیلی {SCHOOL_YEAR or '۱۴۰۵-۱۴۰۶'}","8sp",SECONDARY,False,True))
        out=Button(text=rtl_text("خروج از حساب"),font_name=font_name(),font_size="11sp",background_normal="",background_color=(.65,.12,.14,1),color=WHITE,size_hint_y=None,height=dp(34)); out.bind(on_release=self.logout); content.add_widget(out); root.add_widget(content)
        self.drawer_layer=FloatLayout(size_hint=(1,1),opacity=0,disabled=True); self.drawer_layer.add_widget(Button(background_normal="",background_color=(0,0,0,.28),size_hint=(1,1),on_release=self.close_drawer)); self.drawer=BoxLayout(orientation="vertical",padding=dp(10),spacing=dp(5),size_hint=(None,1),width=dp(280),pos_hint={"right":1}); self.drawer.add_widget(self._label(APP_NAME,"18sp",PRIMARY,True,True))
        from kivy.uix.scrollview import ScrollView
        scroll=ScrollView(do_scroll_x=False); self.drawer_box=BoxLayout(orientation="vertical",spacing=dp(4),size_hint_y=None); self.drawer_box.bind(minimum_height=self.drawer_box.setter("height")); scroll.add_widget(self.drawer_box); self.drawer.add_widget(scroll)
        close=Button(text=rtl_text("بستن منو"),font_name=font_name(),background_normal="",background_color=PRIMARY,color=WHITE,size_hint_y=None,height=dp(38)); close.bind(on_release=self.close_drawer); self.drawer.add_widget(close); self.drawer_layer.add_widget(self.drawer); root.add_widget(self.drawer_layer); self.add_widget(root)
    def role(self):
    ...
    def refresh(self):
        ...
        self.welcome.text=rtl_text(f"خوش آمدید، {name}"); self.role_text.text=rtl_text(f"پنل {ROLE_TITLES.get(role,'کاربر')} | دسترسی فعال"); self.school_text.text=rtl_text(SCHOOL_NAME); self.status.text=rtl_text(f"{len(items)} پنل فعال — برای ورود، کارت پنل را لمس کنید."); self.drawer_box.clear_widgets(); pages=[]
        for i,(title,route) in enumerate(items,1):
            page=PanelCard(size_hint=(1,1),pos_hint={"x":0,"y":0})
            # Transparent full-card hit target: child buttons remain available, but the entire card is a real Kivy Button target.
            hit=Button(text="",background_normal="",background_down="",background_color=(0,0,0,0),color=(0,0,0,0),size_hint=(1,1),pos_hint={"x":0,"y":0})
            hit.bind(on_release=lambda *_args,r=route:self.open(r)); page.add_widget(hit)
            page.add_widget(self._label(title,"23sp",PRIMARY,True,True)); page.add_widget(self._label(f"پنل {i} از {len(items)}","10sp",SECONDARY,False,True)); page.add_widget(self._label(self.desc(route),"11sp",SECONDARY,False,True))
            enter=Button(text=rtl_text("ورود به محیط پنل"),font_name=font_name(),font_size="13sp",background_normal="",background_color=PRIMARY,color=WHITE,size_hint_y=None,height=dp(48)); enter.bind(on_release=lambda *_args,r=route:self.open(r)); page.add_widget(enter); pages.append(page)
        ...
        try:
            ...
            def navigate(_dt):
                try:
                    ...
                    screen=self.manager.get_screen("module"); screen.set_module(route,"dashboard"); self.manager.current="module"
                except Exception as exc:
                    import traceback; traceback.print_exc(); self.status.text=rtl_text("خطا در باز کردن پنل: "+str(exc))
    ...
    def logout(self,*_):
        ...
        if self.manager:self.manager.current="login"
    def toggle_drawer(self,*_): self.drawer_layer.opacity=1; self.drawer_layer.disabled=False; self.drawer_open=True
    def close_drawer(self,*_): self.drawer_layer.opacity=0; self.drawer_layer.disabled=True; self.drawer_open=False