...
class LoginScreen(Screen):
    ...
    def _build(self):
        ...
        root.add_widget(Label(
            text=rtl_text(APP_NAME), font_name=font_name(),
            font_size="36sp", bold=True, color=PRIMARY,
            size_hint_y=None, height=dp(60),
        ))
        root.add_widget(Label(
            text=rtl_text(SYSTEM_TITLE), font_name=font_name(),
            font_size="17sp", color=SECONDARY,
            size_hint_y=None, height=dp(42),
        ))
        root.add_widget(Label(
            text=rtl_text(SCHOOL_NAME), font_name=font_name(),
            font_size="13sp", color=PRIMARY,
            size_hint_y=None, height=dp(38),
        ))
        root.add_widget(Label(
            text=rtl_text("ورود کاربران"), font_name=font_name(),
            font_size="20sp", color=PRIMARY, bold=True,
            size_hint_y=None, height=dp(42),
        ))

        self.identifier = PersianTextInput(
            hint_text=rtl_text("کد ملی"), multiline=False,
            input_filter="int", size_hint_y=None, height=dp(54),
            halign="right", padding=[dp(14), dp(14)],
        )
        self.password = PersianTextInput(
            hint_text=rtl_text("رمز عبور"), password=True,
            multiline=False, size_hint_y=None, height=dp(54),
            halign="right", padding=[dp(14), dp(14)],
        )

        self.status = Label(
            text="", font_name=font_name(), font_size="13sp",
        ...
        self.login_button = Button(
            text=rtl_text("ورود به فراهوش"), font_name=font_name(),
    ...
    def login(self, *_):
        ...
        if not self.app_state.api.configured:
            self._set_status("اتصال سرور تنظیم نشده است.", ERROR)
            return
    ...
    def _authenticate(self, identifier, password):
        try:
            ...
            message = str(exc).strip() or "ورود انجام نشد."
