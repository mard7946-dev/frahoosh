...
class ModuleScreen(Screen):
    ...
    def _load_table(
        ...
        if (
            ...
            self._error(
                "اتصال به سامانه مرکزی آماده نیست."
            )
        ...
        if not self.app_state.api.configured:
            ...
            self._error(
                "تنظیمات اتصال سامانه مرکزی در این نسخه تنظیم نشده است."
            )
    ...
    def _render_rows(
        ...
        if rows is None:
            ...
            self._error(
                "اطلاعات این بخش هنوز در سامانه مرکزی "
                "در دسترس نیست."
            )

            self._add_info(
                "در صورت وجود جدول مربوطه در سامانه مرکزی، "
                "پس از اتصال صحیح سامانه مرکزی اطلاعات نمایش داده می‌شود.",
                110
            )
    ...
    def _payment_message(self):
        ...
        self.status_label.text = rtl_text(
            "درگاه پرداخت باید در سامانه مرکزی مدرسه "
